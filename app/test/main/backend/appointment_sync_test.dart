...
void main() {
  ...
  group('CachedServices', () {
    ...
    test('filters services by clinicId', () async {
      ...
      final results = await (db.select(
        db.cachedServices,
      )..where((s) => s.clinicId.equals(testClinicId))).get();
  ...
  group('CachedAppointments', () {
    test('inserts and reads a guest appointment', () async {
      ...
      )..where((a) => a.userId.equals('guest'))).get();
    ...
    test('deletes appointment by id', () async {
      ...
      await (db.delete(
        db.cachedAppointments,
      )..where((a) => a.id.equals(testAppointmentId))).go();
    ...
    test('updates appointment fields', () async {
      ...
      await (db.update(
        db.cachedAppointments,
      )..where((a) => a.id.equals(testAppointmentId))).write(
      ...
      final result = await (db.select(
        db.cachedAppointments,
      )..where((a) => a.id.equals(testAppointmentId))).getSingle();
    ...
    test('orders appointments by startTime ascending', () async {
      ...
      final results = await (db.select(
        db.cachedAppointments,
      )..orderBy([(a) => OrderingTerm.asc(a.startTime)])).get();
