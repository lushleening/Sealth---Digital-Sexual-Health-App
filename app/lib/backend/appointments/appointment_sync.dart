...
class AppointmentSyncService {
  ...
  Future<void> syncClinics() async {
    ...
    await db.batch((batch) {
      batch.insertAllOnConflictUpdate(
        db.cachedClinics,
        rows.map((r) => CachedClinicsCompanion.insert(
          id: r['id'].toString(),
  ...
  Future<void> syncServices() async {
    ...
    await db.batch((batch) {
      batch.insertAllOnConflictUpdate(
        db.cachedServices,
        rows.map((r) => CachedServicesCompanion.insert(
          id: r['id'].toString(),
  ...
  Future<void> syncAppointments() async {
    ...
    await db.batch((batch) {
      batch.insertAllOnConflictUpdate(
        ...
        rows.map((r) => CachedAppointmentsCompanion.insert(
          id: r['id'].toString(),
  ...
  Future<List<Appointment>> getCachedAppointments(String userId) async {
    final rows = await (db.select(db.cachedAppointments)
  ...
  Future<List<Map<String, dynamic>>> getCachedServices(String clinicId) async {
    final rows = await (db.select(db.cachedServices)
          ..where((s) => s.clinicId.equals(clinicId)))
        .get();
    return rows.map((r) => {
      'id': r.id,
